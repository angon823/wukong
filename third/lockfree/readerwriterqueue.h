...
public:
	...
	bool try_dequeue(U& result) AE_NO_TSAN
	{
        ...
        /// 在出队操作里，只会对front游标写，对tail只会读，
        /// 所以即使没有读到最新的tail（tal写进了数据但是这里恰好没有观察到），
        /// 也不会破坏数据的正确性，最多是这次有数据了但是没有读到，下一次读就可以读到了
		...
		if (blockFront != blockTail || blockFront != (frontBlock_->localTail = frontBlock_->tail.load())) {
		    /// 如果首Block不为空   (Block内：tail==front 为空， tail + 1 == front 为满)
		...
		else if (frontBlock_ != tailBlock.load()) { /// 如果frontBlock == tailBlock 说明读写在一个节点上了，但这个节点已经知道了是空的，所以整个链表都是空的
			fence(memory_order_acquire);

			/// 这段再次检查是为了处理下面描述的这种情况:
			/// 一开始tailBlock == frontBlock, 并且都为空；上面的if判断不通过，进入到else if的判断，
			/// 就在此时生产者在极短的时间内填满了一个以上的Block(包括frontBlock)，然后tailBlock继续前进，所以可以进入到这个else if
			///
			/// 所以如果此时frontBlock游标直接前进到next会导致读的时候跳过了一个已填满的Block
...
private:
...
#endif
	{
		...
		/// 在入队操作里，只会对tail游标写，对front只会读，
		/// 所以即使没有读到最新的front（front被读走了但是这里恰好没有观察到），
		/// 也不会破坏数据的正确性，最多是已经有可写的空间了，但返回了写失败（或开辟了新内存）

		Block* tailBlock_ = tailBlock.load(); /// 从末尾开始写
		size_t blockFront = tailBlock_->localFront;
		size_t blockTail = tailBlock_->tail.load();

		size_t nextBlockTail = (blockTail + 1) & tailBlock_->sizeMask;
		if (nextBlockTail != blockFront || nextBlockTail != (tailBlock_->localFront = tailBlock_->front.load())) {
		    /// 如果尾Block还有空余空间 (Block内：tail==front 为空， tail + 1 == front 为满)
			fence(memory_order_acquire);
		    ...
		    /// 最后一Block没有空闲位置了
			fence(memory_order_acquire);
			if (tailBlock_->next.load() != frontBlock) { /// tailBlock.next==frontBlock 说明 链表也没有余粮了
			    /// 但前面的Block可能已经被读掉了(frontBlock游标向后前进了)，所以还有可能可以写
				...
				/// 新的这个块肯定是空的，否则frontBlock不会前进到它的next
				assert(nextBlockFront == nextBlockTail);
				tailBlockNext->localFront = nextBlockFront; /// 多余赋值？
			...
			else if (canAlloc == CanAlloc) { /// tailBlock.next==frontBlock 链表也没有余粮了
				// tailBlock is full and there's no free block ahead; create a new block
				auto newBlockSize = largestBlockSize >= MAX_BLOCK_SIZE ? largestBlockSize : largestBlockSize * 2; /// 把块的大小增长一倍，尽量减少内存分配次数
